...
export default class Race {
  constructor(_options = {}) {
    ...
    this.loadertextures.load(
      '/minijuegos/bike-race/graderia.jpg',
      (texture) => {
    ...
    this.loadertextures.load(
      '/minijuegos/bike-race/wildpork1.png',
      (texture) => {
    ...
    this.loadertextures.load(
      '/minijuegos/bike-race/beachball.png',
      (texture) => {
  ...
  init() {
    ...
    const sunLight = new THREE.DirectionalLight(0x4455ff, 2);
  ...
  createCityWorld() {
    ...
    for (let i = 0; i < roadPoints.length - 1; i++) {
      ...
      leftWall.position.set(
        segment.position.x - perpX,
        0.6,
        segment.position.z - perpZ,
      );
      ...
      if (i % 10 === 0 && i < roadPoints.length - 20) {
        this.createBuilding(
          segment.position.x - perpX * 3,
          segment.position.z - perpZ * 3,
        );
        this.createBuilding(
          segment.position.x + perpX * 3,
          segment.position.z + perpZ * 3,
        ...
        this.createPalmTree(
          segment.position.x - perpX * 1.5,
          segment.position.z - perpZ * 1.5,
        );
      }
    }
  }
  ...
  createPalmTree(x, z) {
    ...
    const trunk = new THREE.Mesh(
      new THREE.CylinderGeometry(0.15, 0.3, 6),
      new THREE.MeshPhongMaterial({ color: 0x6e4a2e }),
    );
    trunk.position.y = 3;
    group.add(trunk);
  ...
  createFootballStadium() {
    ...
    postL.position.set(-10, 4, 70);
    goalGroup.add(postL);
    const postR = new THREE.Mesh(postGeo, postMat);
    postR.position.set(10, 4, 70);
    goalGroup.add(postR);
    const bar = new THREE.Mesh(new THREE.BoxGeometry(20, 0.4, 0.4), postMat);
    bar.position.set(0, 8, 70);
    goalGroup.add(bar);
    ...
    const standMat = new THREE.MeshPhongMaterial({
      map: this.textures.graderia,
    }); // Azul Barcelona
    const standL = new THREE.Mesh(standGeo, standMat);
    standL.position.set(-65, 10, 0);
    stadiumGroup.add(standL);
    const standR = new THREE.Mesh(standGeo, standMat);
    standR.position.set(65, 10, 0);
    stadiumGroup.add(standR);
    ...
    const metaBanner = new THREE.Mesh(
      new THREE.BoxGeometry(this.roadWidth * 2, 5, 0.5),
      new THREE.MeshPhongMaterial({ map: this.textures.tarr }),
    );
  ...
  updatePhysics() {
    ...
    if (this.gameState === 'racing') {
      ...
      if (this.keys['arrowleft'] || this.keys['a'])
        this.player.rotation += this.player.turnSpeed;
      ...
      if (
        collision.distance > this.roadWidth - 1 &&
        this.playerBike.position.z < 1000
      ) {
      ...
      const aiLookAt = this.curve.getPoint(
        Math.min(1, this.ai.progress + 0.01),
      );
      ...
      this.camera.position.lerp(
        this.playerBike.position.clone().add(camOffset),
        0.1,
      );
      this.camera.lookAt(
        this.playerBike.position.x,
        1,
        this.playerBike.position.z + 2,
      );
  ...
  updateUI() {
    if (this.gameState === 'racing' && this.playerBike) {
      this.timerDisplay.innerText = this.formatTime(
        Date.now() - this.startTime,
      );
      const prog = Math.min(
        100,
        Math.floor((this.playerBike.position.z / 1000) * 100),
      );
      this.progressDisplay.innerText = `${prog}%`;
    }
    this.speedDisplay.innerText = Math.floor(this.player.velocity * 200);
    let pos =
      this.playerBike.position.z >= this.aiBike.position.z ? '1º' : '2º';
